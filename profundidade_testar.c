...
Observação:
fazer 2 execuções
em uma delas é para observar o comportamento (a cada 50 execuções imprime o tabuleiro)
na outra é para comentar a parte da impressão (eu indiquei por "COMENTAR" no código)
para medir o tempo, pois o tempo imprimindo o tabuleiro a cada 50 execuções é muito diferente
...
#include <stdlib.h>
//inclui função para medir o tempo de execução
#include <time.h>
